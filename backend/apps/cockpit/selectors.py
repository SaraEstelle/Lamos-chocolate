...
def get_pulse():
    ...
    revenue_today = (
        paid.filter(created_at__date=today).aggregate(t=Sum("total_amount"))["t"] or 0
    )
    revenue_mtd = (
        paid.filter(created_at__date__gte=month_start).aggregate(t=Sum("total_amount"))[
            "t"
        ]
        or 0
    )
...
def get_product_detail(product_id, days=90):
    ...
    stock_map = {
        s["sku__sku_code"]: s
        for s in Stock.objects.filter(sku__product_id=product_id).values(
            "sku__sku_code", "quantity", "threshold_alert"
        )
    }
...
def get_pilotage(days=30):
    ...
    targets = {t.product_id: t.monthly_units for t in ProductTarget.objects.all()}
...
def get_orders_by_weekday(days=90):
    ...
    bars = [
        {"label": _WEEKDAY_LABELS[i], "count": counts.get(i + 1, 0)} for i in range(7)
    ]
...
def get_orders_by_hour(days=90):
    ...
    peak = (
        max(bars, key=lambda b: b["count"]) if any(b["count"] for b in bars) else None
    )
    return {
        "days": days,
        "bars": bars,
...
def get_funnel(days=90):
    ...
    return {
        "days": days,
        "stages": stages,
        "overall_conversion": (round(raw[-1][1] / top * 100, 1) if top else 0.0),
    }
...
def get_top_viewed(days=90, limit=5):
    ...
    rows = list(
        Event.objects.filter(
            event_type=EventTypeChoices.PRODUCT_VIEW, created_at__gte=since
        )
