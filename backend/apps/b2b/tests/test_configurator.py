...
def pro_with_sku(sample_product):
    ...
    c = Customer.objects.create_user(
        ...
        customer_type="b2b_hospitality",
    )
    account = B2BAccount.objects.create(
        customer=c,
        company_name="Hotel X",
        status="active",
    )
...
class TestConfigurator:
    ...
    def test_request_quote_creates_customization_and_email(
        self, client, pro_with_sku, mailoutbox
    ):
        c, account, sku = pro_with_sku
        client.force_login(c)
        resp = client.post(
            ...
            data={
                ...
                "logo_engraved": "on",
                "inner_packaging": "Gold box",
            },
        )
        assert resp.status_code == 302
        assert (
            CustomizationRequest.objects.filter(
                account=account,
                status="quote",
            ).count()
            == 1
