...
from apps.b2b.models import B2BProductInfo, B2BRequest
...
def has_recent_duplicate(*, contact_email, company_name, within_minutes=10):
    ...
    return B2BRequest.objects.filter(
...
def get_pro_catalogue():
    ...
    """
    return (
        B2BProductInfo.objects.filter(is_b2b_available=True, sku__is_active=True)
...
def get_b2b_orders(account, limit=20):
    ...
    """
    return (
        Order.objects.filter(customer=account.customer, channel="b2b")
